...
## len(departure[]) 不可以超过6， N 不可以超过6 
#  先 N+1 或 N-1 之后再去检验 N 是否大于6 
#  departure[] 长度为0时，队列为空，所以是drate*1
...
while (t<T and len(arrival)>0):  
    if len(departure)==0:          # 离开队列为0，等待到达
        ...
        if departure[0]<arrival[0]: # 排队的人减少,不需要排队， N=N-1
            ...
            if N>=2:                # 如果队列不为空，算出下一个离去的时间
                de=t+np.random.exponential(1.0/(2*drate))  
                departure.append(de)
                departure.sort()  # 从小到大排序，小时刻先出
            elif N==1:
                de=t+np.random.exponential(1.0/(1*drate))  
                departure.append(de)
                departure.sort()  # 从小到大排序，小时刻先出
        # 在下一个到达后再离去
